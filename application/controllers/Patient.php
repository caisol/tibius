...
class Patient extends CI_Controller {
	...
	public function managePatient()
	{
		...
		$query_patients = $this->db->query("SELECT p.`id`,
									  p.`member_id`,
									  p.`first_name`,
									  p.`middle_name`,
									  p.`last_name`,
									  ...
									  p.`gmiddle_name`,
									  p.`glast_name`,
									  p.`phone_type_preferred`,
									  p.`email`,
									  p.`gphone_type_preferred`,
									  p.`gemail`,
									  p.`pcheck`, p.`department`, p.`status`, p.`created_at`,d.id as department_id,d.department_name,
									  p.`updated_at`  FROM `mz_patients` p LEFT JOIN mz_departments d ON d.id=p.department WHERE p.status=1");
		...
		$queryD = $this->db->order_by('id', 'DESC')->get_where(DB.'departments',array('status'=>1));
		$departments = $queryD->result();
		
		$data['departments'] = isset($departments)?$departments:array();

		
		
		$user_id = $this->session->userdata('user_id');
		
		$this->template->load('templates/template_front_home_v2', 'front/manage_patients_v2',$data);
	}
		...
		if ($data['status'] === TRUE)
		{
			if(isset($patient_id) && $patient_id>0)
			{
				...
				if($this->db->update(DB.'patients'))
				{
					...
					$sql_patient_detail_delete = "DELETE FROM ".DB."patients_detail WHERE patient_id=".$patient_id;
					if($this->db->query($sql_patient_detail_delete))
					{
...
public function getPatient()
	{
		$patint_id   = $this->input->post("pid");
		$data['status'] = "failure";
		$data['data'] = array();
		if(isset($patint_id) && !empty($patint_id))
		{
			$record = $this->db->get_where(DB.'patients',array('status'=>1,"id"=>$patint_id))->result_array();
			
			if(isset($record[0]) && isset($record[0]["id"]) && $record[0]["id"]>0)
			{
				$record = $record[0];
	...
	public function manageDepartments()
	{
		...
		$query = $this->db->order_by('id', 'DESC')->get_where(DB.'departments',array('status'=>1));
		...
		$this->template->load('templates/template_front_home_v2', 'front/manage_departments',$data);
	}
	...
	public function submitDepartment()
	{
		...
		$department_hidden_id   = $this->input->post("department_hidden_id");
		$department_id   = $this->input->post("department_id");
		$this->form_validation->set_rules("department_name","Department Name","trim|required|max_length[200]");
		$this->form_validation->set_rules("department_hidden_id","Department ID","trim|max_length[200]");
		$data['status'] = $this->form_validation->run();
		...
		if ($data['status'] === TRUE)
		{
			if(isset($department_id) && $department_id>0)
			{
				...
				$this->db->set('department_id', $department_hidden_id);
				$this->db->set('updated_at', __date_time);
				$this->db->where('id', $department_id);
				if($this->db->update(DB.'departments'))
				{
					$data['department_id'] = $department_hidden_id;
					$data['status'] = true;
			...
			{
				$post_data = array('department_name'=> $department_name,
				'department_id'=> $department_hidden_id,
				'created_at'=>__date_time);
				
				
				if($this->db->insert(DB.'departments',$post_data))
				{
	...
	public function getDepartmentDetail()
	{
		...
		{
			$record = $this->db->get_where(DB.'departments',array('status'=>1,"id"=>$department_id))->row();
	...
	public function delete_department()
	{
		...
		if(isset($id) && $id>0)
		{
			$record = $this->db->get_where(DB.'departments',array('status'=>1,"id"=>$id))->row();
			if(isset($record) && !empty($record))
			{
				$this->db->set('status', 0);
				$this->db->set('updated_at', __date_time);
				...
				if($this->db->update(DB.'departments'))
				{
					redirect('manage-departments?msg=Success');
				}
				else
				{
					redirect('manage-departments?msg=Error');
				}
			}
		}
	}
